########### ANNOTATION WITH singleR - multiple REFS ################
####################################################################


# script to annotate cell types from 20k Human PBMCs from a healthy female donor

# Various strategis to preform cell type annotation using multiple references (combined) are described
...
hpca <- celldex::HumanPrimaryCellAtlasData() # Microarray datasets from human primary cells--> blood sub population of cell types
dice <- celldex::DatabaseImmuneCellExpressionData() # Bulk-RNA seq data from immune cell population
...
# adding ref info to labels to distinguish from references
...
# save into the metadata slot of the seurat object
...
#remove the strings that distinguish from the 2 references
hpca$label.main
dice$label.main
